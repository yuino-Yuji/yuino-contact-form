...
**CC への依頼例**：`「フォーム fields の構成に合わせて、管理者通知と自動返信のメール雛形を option_ycf_settings フィルターで案件側に書いて」`
