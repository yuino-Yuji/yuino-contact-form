...
- **管理画面**：4 ステップのセットアップ案内で「次に何をすればいいか」を可視化（納品後にクライアントも見るため、Claude Code には言及しない）

---

## クイックスタート（Claude Code を使う場合の標準フロー）

1. このリポジトリを `wp-content/plugins/yuino-contact-form/` に配置
2. WordPress 管理画面 → プラグイン → 「Yuino Contact Form」を有効化
3. 「設定 → お問い合わせ設定」を開く → **4 ステップのセットアップ案内** が現在地を表示
4. 各 STEP に対応する次の依頼文を **Claude Code にコピペで投げる**（管理画面には依頼文を表示しないので、この README を参照する）：
   ...
   - STEP 3：`wp-config.php` に SMTP パスワードの 1 行を **ユーザーが直接追記**（後述の理由により Claude Code は触らない）
   - STEP 4：通常は STEP 1 で完了済み。必要なら `「フォーム内容とサイト情報に合わせて、管理者通知と自動返信のメール件名・本文を整えて」` で微調整
5. `✅ 運用準備完了` パネルが出れば運用開始

Claude Code を使わない手動セットアップは末尾「10. Claude Code を利用しない場合の手動セットアップ」を参照。
...
| `tel` | 電話番号（全角数字・全角/各種ハイフン・空白・丸括弧を**自動で半角へ正規化**したうえで、**半角数字とハイフン(-)のみ**を検証。それ以外の文字は弾く） |
| `textarea` | 複数行テキスト |
...
- **4 ステップのセットアップ案内**（ページ上部）：現在地を表示し、次の一手を案内
...
- **高機密情報**（`wp-config.php` に直接記述）：
  ...
  ```
  ※ **ユーザー自身が直接編集**（Claude Code のチャット履歴に SMTP パスワードを残さないため、本プラグインの推奨フローでは Claude Code に依頼せず手動編集）
...
**GPL-2.0-or-later**（全文は [LICENSE](LICENSE) を参照）

Copyright (C) 2026 Yuino

WordPress 本体が GPL であるため、本プラグインも GPL-2.0-or-later で配布しています。
複製・改変・再配布は自由ですが、著作権表示とライセンス表示は保持してください。

### サポート方針

**本プラグインは、Yuino が自社の受託案件で使うために開発しているものです。**
公開しているのは自社案件への配布（自動更新）を成立させるためであり、
広く使ってもらうことを目的としていません。

- 外部の利用者に対するサポート・動作保証は行いません
- Issue / Pull Request への対応をお約束できません
- 自社案件の都合で、予告なく仕様変更・破壊的変更を行うことがあります
- 利用は自己責任でお願いします（GPL 第 11 条・第 12 条のとおり**無保証**です）

---

## 10. Claude Code を利用しない場合の手動セットアップ

本プラグインは Claude Code ファースト設計だが、手動でも一通りセットアップ可能。
...
- 管理画面の 4 ステップのセットアップ案内は手動の手順で書かれているので、そのまま従って進めればよい（「✅ 運用準備完了」と表示されれば運用可能）
...
YCF を別のサイト（自分以外の案件・知人案件等）に導入してもらう場合、**Claude Code に丸投げできる**ように設計されている。

### Claude Code を使う場合（推奨・最短）

導入先サイトの Claude Code に、以下のメッセージを送るだけで完了する：
...
5. 「設定 → お問い合わせ設定」の 4 ステップのセットアップ案内に誘導

その後は本 README 上段「クイックスタート」のフローに乗ればよい（fields 登録の依頼文はクイックスタートに掲載）。

### Claude Code を使わない場合（手動）
