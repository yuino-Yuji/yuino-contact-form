...
> **v0.2.13 で方針変更**：管理画面は納品後にクライアントも目にするため、画面上から Claude Code / CC への言及と依頼文 blockquote を全て削除し、開発ツールに依存しない手順説明に置き換えた。依頼文の正本は README.md「クイックスタート」。下表の「案内内容」列は v0.2.12 以前の記録として残す。
...
- **README.md に「12. 他の WordPress サイトに導入する」セクション新設** — Claude Code（CC）に丸投げできる導線テンプレ（依頼文）、CC を使わない場合の手動セットアップ手順、導入後の自動更新運用、共有時チェックリストを整備
- ユーザーから「他の人に YCF を使ってもらう場合、どう案内したらよいか」という質問が出発点。プラグイン自体は CC ファースト設計なので、**配布も「リポ URL を渡す + 依頼文テンプレを CC に投げる」だけで完結する形**に整理した
- コードベースの変更はなし（README.md 整備とバージョン番号同期のみ）

#### 設計判断ポイント

- **「中レベル整備」を選択** — 自前リリースアセット zip 添付（事前リネーム済み `yuino-contact-form.zip`）や GitHub Actions による自動化までは踏み込まない方針。受け手が CC ユーザーである前提なら、source archive のままで CC がリネーム処理を担えるため、毎リリースの手作業を増やしてまでアセット zip を整備する必要がない（YAGNI）
- **依頼文テンプレを `> ブロック引用` で囲った** — README から **そのままコピペで CC に投げられる**形に。読み手が「どこから貼ればいいか」を迷わない
- **共有時チェックリストを「渡すべき情報」「受け手環境の前提」2 列で構造化** — 配布時のオペレーションが「コピペできる」レベルまで分解されている

#### 残作業

タグ push → `gh release create v0.2.7` → 動作確認は不要（README 整備のみのため）

### v0.2.13（2026-09-18）

- **管理画面から Claude Code への言及を全削除** — セットアップ案内 STEP 1〜4・フォーム未登録時の案内・「✅ 運用準備完了」パネルを、開発ツールに依存しない手順説明に書き換え。Claude Code 向けの依頼文は README.md「クイックスタート」に一本化
- **STEP 2 の「Claude Code に『SMTP 情報を設定して』と依頼することも可能」を削除** — STEP 3 の「SMTP パスワードを Claude Code のチャット履歴に残さない」方針と矛盾し、依頼の流れでパスワードまで貼られる危険があったため
- README.md / readme.txt の略記「CC」を「Claude Code」に統一（メールの CC 欄は除く）

#### 設計判断ポイント

- **管理画面＝クライアントも見る画面、マニュアル＝制作者が読む文書、と役割を分けた** — 完了後の画面に常時「Claude Code に依頼してください」と出るのは、納品物として不自然。制作者向けの導線はマニュアル側に置く
- **「CC」の略記をやめた** — 同じ設定画面にメールの「CC・BCC」欄があり紛らわしいため
- readme.txt はプラグイン一覧の「詳細を表示」モーダルにも出るが、説明書（マニュアル）扱いとして Claude Code への言及を残した

### v0.2.15（2026-09-26）

- **送信ガードが、止められた送信でもボタンを無効化していた不具合を修正** — `initSubmitGuard()` は submit のたびに `setTimeout(0)` でボタンを `disabled` / `is-loading` にしていた。テーマ側の追加検証が `preventDefault()` で送信を止めてもボタンだけが無効のまま残り、**入力を直しても再送信できない**（再読み込みが必要）状態になっていた。`setTimeout` の中で `event.defaultPrevented` を見て、止められた送信では無効化しない
- **bfcache 復元時にボタンを戻す** — 「戻る」でキャッシュから復元されたページでは、送信時に無効化したボタンがそのまま残るため、`pageshow`（`persisted`）で元に戻す
- **`form` 属性で結び付けた送信ボタンにも二重送信防止を効かせる** — ガードは `form.querySelector('button[type="submit"]')` で探していたため、ボタンを `<form>` の外に置いて `form="…"` で結び付けたテンプレート（mysite2401 の確認画面）では防止が効いていなかった。`event.submitter` → `form.elements` の順で探す（`form.elements` は form 属性で結び付いた要素も含む）
- `YCF_VERSION` 定数が 0.2.13 のままだったのを 0.2.15 に揃えた（v0.2.14 でヘッダーだけ上げていた）

#### 設計判断ポイント

- **判定は `setTimeout` の中で行う** — submit のハンドラーの中で `event.defaultPrevented` を見ても、後から登録されたハンドラー（テーマ側の検証）はまだ実行されていない。`setTimeout(0)` の時点では全ハンドラーが終わっているので、値が確定している
- **発見の経緯** — mysite2401 の WCAG 監査で、「ご担当者様」に英字を入れて JS 検証エラーを出すと送信ボタンが戻らないことを実機で確認した。YCF でテーマ側の検証を足すすべての案件に影響するため、テーマでの回避ではなく本体で直した
